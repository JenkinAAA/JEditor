...
export function createColorPopover(config, ctx) {
  ...
  config.showCustomPanel = (section, mode, localCtx) => {
    ...
    customPanel.querySelector('.je-color-custom-title').textContent =
      mode === 'text' ? '自定义字体颜色' : '自定义背景颜色'
    customPanel.querySelector('.je-color-picker').value = config.getCurrentColor(
      localCtx.editor,
      mode,
    )
