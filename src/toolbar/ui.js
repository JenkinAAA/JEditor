...
function createDropdownButton(plugin) {
  ...
  btn.className =
    hasIcon && hasText
      ? 'tool-btn-text tool-btn-text--icon'
      : hasIcon
        ? 'tool-btn'
        : 'tool-btn-text'
...
function positionPopover(popover, anchorEl, side = 'bottom') {
  ...
  const top = side === 'right' ? rect.top + window.scrollY : rect.bottom + window.scrollY + 8
  const left =
    side === 'right' ? rect.right + window.scrollX + 8 : Math.max(12, rect.left + window.scrollX)
...
function updateRegularButton(containerEl, plugin, state) {
  ...
  const button = containerEl.querySelector(
    `[data-command="${plugin.name}"], [data-command-toggle="${plugin.name}"]`,
  )
  if (!button) return
...
export function initToolbarEvents(containerEl, editor, pluginManager, options = {}) {
  ...
  function normalizeSecondaryOverflow() {
    ...
    overflowOrder.forEach((name) => {
      const element = secondaryRow.querySelector(
        `[data-plugin-name="${name}"], [data-command-group="${name}"]`,
      )
    ...
    const measureUsedWidth = () =>
      Array.from(secondaryRow.children).reduce((total, child) => {
